...
function stableSortedStringify(value: unknown): string {
  ...
  if (typeof value === "object") {
    ...
    const parts = keys.map(
      (key) => `${JSON.stringify(key)}: ${stableSortedStringify(record[key])}`,
    );
    return "{" + parts.join(", ") + "}";
  }
...
/**
 * Simple SHA-1 hex digest. Uses Node.js crypto when available,
 * falls back to Web Crypto (which is available in Next.js edge and browser).
 */
...
function assistantToolUseEvent(
  ...
  const normalizedInput =
    toolInput && typeof toolInput === "object" && !Array.isArray(toolInput)
      ...
      : {};
  const resolvedId =
    asString(toolCallId).trim() || stableToolCallId(toolName, normalizedInput);
  return {
    type: "assistant",
    message: {
      content: [
        { type: "tool_use", id: resolvedId, name: toolName, input: normalizedInput },
      ],
    },
  };
}
...
function usagePayloadFromSource(
  ...
  const model =
    asString(message.model) || asString(source.model) || null;
  return [usage, model];
}
...
function normalizeAmpLikeEvent(event: Record<string, unknown>): CanonicalEvent[] {
  ...
  if (eventType === "user") {
    ...
    for (const block of asList(message.content)) {
      ...
      const toolUseId =
        asString(blockDict.tool_use_id) || asString(event.parent_tool_use_id);
  ...
  if (eventType === "stream_event") {
    ...
    if (nestedType === "error") {
      const msg =
        asString(asRecord(nested.error).message) || "Unknown error";
...
function normalizeCodexItem(
  ...
  if (itemType === "file_change" && phase === "completed") {
    ...
    return [
      { type: "file_change", changes: Array.isArray(changes) ? changes : [] },
    ];
  }
...
function normalizeCodexEvent(event: Record<string, unknown>): CanonicalEvent[] {
  ...
  if (eventType === "turn.failed") {
    ...
    const message =
      asString(error.message) || asString(event.message) || "Turn failed";
    return [{ type: "error", error: message }];
  }
  ...
  if (eventType === "item.started") {
    return normalizeCodexItem(asRecord(event.item), "started");
  }
  if (eventType === "item.updated") {
    return normalizeCodexItem(asRecord(event.item), "updated");
  }
  if (eventType === "item.completed") {
    return normalizeCodexItem(asRecord(event.item), "completed");
  }
...
function normalizePiMessageContent(
  ...
  for (const block of content) {
    ...
    if (blockType === "text") {
      ...
      const toolName =
        asString(toolCall.name) || asString(blockDict.name) || "tool";
      const toolInput = (() => {
...
function normalizePiEvent(event: Record<string, unknown>): CanonicalEvent[] {
  ...
  if (eventType === "tool_execution_start") {
    ...
    if (toolName.trim().toLowerCase() === "subagent") {
      const label =
        ...
        "Delegated subagent";
      return [
        subagentEvent({ status: "started", subagent_id: toolId, name: label }),
      ];
    }
    return [assistantToolUseEvent(toolId, toolName, toolInput)];
  }
  ...
  if (eventType === "message_end") {
    ...
    const normalized = attachUsageMetadata(
      normalizePiMessageContent(message),
      message,
    );
    const stopReason = asString(message.stopReason);
    if (stopReason === "error" || stopReason === "aborted") {
      const errorText =
        asString(message.errorMessage) || "Assistant run failed";
  ...
  if (eventType === "agent_end") {
    ...
    const assistantMessages = messages.filter(
      (m) => asString(asRecord(m).role) === "assistant",
    );
    ...
    if (stopReason === "error" || stopReason === "aborted") {
      const errorText =
        asString(lastAssistant.errorMessage) || "Assistant run failed";
