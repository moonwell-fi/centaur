...
_EXECUTION_HARNESSES = {"amp", "claude-code", "codex", "pi-mono"}
...
def _resolve_prompt_selector(
    ...
    if not prompt_selector:
        return {"harness": None, "persona_id": None}
    normalized = _PROMPT_FLAG_ALIASES.get(prompt_selector, prompt_selector)
    if normalized in _EXECUTION_HARNESSES:
        return {"harness": normalized, "persona_id": None}
    return {"harness": None, "persona_id": normalized}
