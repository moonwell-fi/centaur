...
def main(
    account: Optional[str] = typer.Option(
        None, "--account", "-a", help="Google account email to use"
    ),
):
    """GSuite CLI - use --account to specify which Google account."""
    if account:
        from .client import set_account
...
@app.command()
def auth():
    """Authenticate with Google (OAuth flow)."""
    from .client import get_credentials, _current_account

    try:
        get_credentials()
        console.print("[green]✓ Authenticated successfully[/]")
        if _current_account:
            console.print(f"[dim]Token saved to ~/.config/gsuite/tokens/{_current_account}.json[/]")
        else:
            console.print("[dim]Token saved to ~/.config/gsuite/token.json[/]")
